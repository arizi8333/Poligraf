...
<!-- <div class="row">
    ...
    <div class="modal fade" id="modal" tabindex="-1" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="row">
                <div class="col mb-3">
                <label for="nameBasic" class="form-label">Layanan Pemeriksaan</label>
                <input type="text" name="layanan" id="layanan" class="form-control" />
                </div>
            </div>
            <div class="row">
                <label for="nameBasic" class="form-label">Harga Layanan</label>
...
<script>
    ...
    $(document).on('click', '#add', function() {
        ...
        $('#layanan').val("");
        $('#harga').val("");
        $('#form').attr('action', '{{ url('admin/layanan/create') }}');     
    });
    ...
    $(document).on('click', '#edit', function() {
        ...
        $.ajax({   
            type: "get",
            url: "{{ url('/admin/layanan/edit') }}/"+id,
            dataType: "json",
            success: function(data) {
                console.log(data[0].id);
                event.preventDefault();
                var layanan=data[0].nama_layanan
                var harga=data[0].harga
                
                $('#layanan').val(layanan).change();
                $('#harga').val(harga).change();
                $('#form').attr('action', '{{ url('admin/layanan/update') }}/'+id);
            }
        });        
    });
    ...
    $(document).on('click', '#delete', function() {
            ...
            if (confirm("Anda Yakin ingin menghapus data?")){
                $.ajax({
                    url : "{{ url('admin/layanan/delete') }}/"+id,
                    success :function () {
                        location.reload();
                    }
                })
    ...
    $( "#admin-menu-layanan" ).addClass( "active" );
});
</script>
@endsection