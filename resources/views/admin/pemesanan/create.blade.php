                    ...
                    <div class="row">
                        <div class="col-lg-4">
                            ...
                            {{ csrf_field() }}
                                ...
                                <label for="nameBasic" class="form-label">Nama Client</label>
                                <select id="client" name="client" class="form-control">
                                    @foreach($users as $user)
                                        <option value="{{$user->id}}">{{$user->nama}} - {{$user->companies}}</option>
                                    @endforeach
                                    <option disabled></option>
                                    <option value="0" class="text-center badge bg-secondary fs-7 fw-bold"">
                                        <a href=""> + Create New +</a>
                                    </option>
                                </select>
                                ...
                                <select id="diskon" name="diskon" class="form-control">
                                    ...
                                    @foreach($diskons as $diskon)
                                        <option value="{{$diskon->id}}">{{$diskon->keterangan}} - {{$diskon->jumlah}} %</option>
                                    @endforeach                                
                                </select>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    $('#form').attr('action', '{{ url('admin/pemesanan/store') }}');
    ...
    $("#client").change(function(){
        ...
        var val = $("#client").val();
        if(val == 0){
            window.location.href = "{{url('admin/client/index')}}";
        }
    ...
    function jadwal_awal(tgl_mulai){
        $.ajax({
            type: 'GET',
            url: '{{url("admin/pemesanan/jadwal")}}/'+tgl_mulai,
            dataType: "json",
    ...
    $.ajax({
            type: 'GET',
            url: '{{url("admin/pemesanan/jadwal")}}/'+tgl_akhir,
            dataType: "json",
            success :function (data) {
                console.log(data[0]);
                if(data[0] < data[1]){
